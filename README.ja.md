...
|---|---|---|
| 「AI コーディング copilot、8 eng + 1 PM、6 ヶ月 roadmap で月次リリース、次の milestone は cross-session memory、launch readiness をどう評価？」<br>→ [`/ai-pm`](.claude/skills/ai-pm/) · Mira | *"Great roadmap! Define success metrics, A/B test memory rollout, monitor adoption KPIs, gather user feedback iteratively."* | 月次リリース + 6 ヶ月 roadmap は 0→1 AI 製品の launch-readiness 一番目の赤信号 — **ship-to-understand** (Turley) の下では月に 1 回しか post-launch 信号を買えない。まず逆問：cross-session memory は本物のユーザー問題か、あなたが提案した解決策か？短コンテキスト + retrieval で足りないと検証済みか？このステップを飛ばして持続的 memory に直行するのは **premature memory commitment** anti-pattern。Launch readiness は 3 つで評価：① **eval as Schelling point** (Karina) — 他者を orient できる task-level eval を書く、「覚えている」ではなく；② **auto-improve moat test** (Krieger) — Claude が升级した時 memory 層は良くなるのか書き直しが要るのか？後者なら feature であって product ではない；③ **harness** (Lopopolo) — 8 eng のうち何人が regression test を書いているか、Slack で「感覚的に精度上がった」と議論していないか？逆提案：次の milestone を 2 週で painted-door + retrieval baseline + regression eval に変更、design partner 3 社で 2 週 agent log を回してから持続的 memory への不可逆投資を決める。 |
...
**差は「より良い prompt」から来ているのではない** — 各 skill は stance + workflow を内蔵し、critique-rubric が構造化された判断基準を与え、anti-patterns が *"premature memory commitment" / "Homogenization to trend" / "validate-the-design" / "frequency-driven calendar" / "deceptively-easy survey"* を **具名失敗モード**（識別 + 悪い案を退ける道具）として列挙する；正のスタンスも anchor 化されている — *"ship-to-understand" (Turley) / "eval as Schelling point" (Karina) / "auto-improve moat test" (Krieger) / "harness as leverage" (Lopopolo)* はすべて各 skill の `references/source-material/` に verified ソースがある。**prompt で即興されたものではなく、skill 定義に焼き込まれた anchor** — 詳細は [How it works](#how-it-works) を参照。
