...
|---|---|---|
| "AI coding copilot, 8 eng + 1 PM, 6-month roadmap on monthly release cadence, next milestone adds cross-session memory — how do I evaluate launch readiness?"<br>→ [`/ai-pm`](.claude/skills/ai-pm/) · Mira | *"Great roadmap! Define success metrics, A/B test memory rollout, monitor adoption KPIs, gather user feedback iteratively."* | Monthly release on a 0→1 AI product = launch-readiness red flag #1: under **ship-to-understand** (Turley) you buy 1 post-launch signal per month. First reframe: is cross-session memory a real user problem or your proposed solution? Verified short-context + retrieval doesn't suffice? Skipping that = **premature memory commitment** anti-pattern. Evaluate on 3 things: ① **eval as Schelling point** (Karina) — task-level eval that would orient others, not "remembers stuff"; ② **auto-improve moat test** (Krieger) — does this memory layer improve when Claude upgrades, or need rewriting? Latter = feature, not product; ③ **harness** (Lopopolo) — of 8 eng, how many writing regression tests vs Slack debates? Counter: change milestone to a 2-week painted-door + retrieval baseline + regression eval, run with 3 design partners × 2 weeks of agent logs, then decide on the irreversible investment into persistent memory. |
...
**The gap doesn't come from "a better prompt."** Each skill ships with stance + workflow; the critique-rubric gives structured judgment standards; anti-patterns list *"premature memory commitment" / "Homogenization to trend" / "validate-the-design" / "frequency-driven calendar" / "deceptively-easy survey"* as **named failure modes** (identification + rejection tools); positive stances are also anchored — *"ship-to-understand" (Turley) / "eval as Schelling point" (Karina) / "auto-improve moat test" (Krieger) / "harness as leverage" (Lopopolo)* all have verified sources in each skill's `references/source-material/`. **These aren't improvised in the prompt — they're fixed anchors in the skill definition** — see [How it works](#how-it-works).
